...
class Evaluator(object):
    def __init__(self, dataset, class_num, norm_mean, norm_std, network, multi_scales, 
                is_flip, devices, verbose=False, save_path=None, show_image=False, config=None, bands_idx=None):
        self.eval_time = 0
...
class EvaluatorV1(object):
    ...
    def add_batch(self, gt_image, pre_image): #* gt_image = Ground truth image
...
class MyConfusionMatrix():
    ...
    def reset(self):
        if self.ignore_index is not None:
            self.tp = torch.zeros(self.num_classes-1).to(torch.long).to(self.device)
            self.fp = torch.zeros(self.num_classes-1).to(torch.long).to(self.device)
            self.tn = torch.zeros(self.num_classes-1).to(torch.long).to(self.device)
            self.fn = torch.zeros(self.num_classes-1).to(torch.long).to(self.device)
        else:
            self.tp = torch.zeros(self.num_classes).to(torch.long).to(self.device)
            self.fp = torch.zeros(self.num_classes).to(torch.long).to(self.device)
            self.tn = torch.zeros(self.num_classes).to(torch.long).to(self.device)
            self.fn = torch.zeros(self.num_classes).to(torch.long).to(self.device)

        self.cm = ConfusionMatrix(task='multiclass', num_classes=self.num_classes).to(self.device)
        self._accuracy = 0.
        self._precision = 0.
        self._recall = 0.
        self._f1_score = 0

def initialize_metrics(device = 'cuda'):
    '''
