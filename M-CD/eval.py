...
from engine.evaluator import Evaluator, EvaluatorV1, initialize_metrics
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device, config):
        As = data['S2_before_image'][:, self.bands_idx, :, :]
        Bs = data['S2_after_image'][:, self.bands_idx, :, :]
        label = data['label']
...
class FlogaEvaluator(object):
    def __init__(self, device, loader, cfg, model):
        self.device = device
        self.evaluator = EvaluatorV1(num_class=2)
    ...
    def __call__(self):
        ...
        with torch.no_grad():
            self.cm.reset()
            self.iou.reset()
            for minibatch in tqdm(self.data_loader):
                # Load images
                As = minibatch['S2_before_image'][:, self.cfg.bands_idx, :, :]
                Bs = minibatch['S2_after_image'][:, self.cfg.bands_idx, :, :]
                gts = minibatch['label']
                ...
                score = torch.exp(self.model(As, Bs))

                preds = score.argmax(1).to(dtype=torch.int8)
                self.cm.compute(preds, gts)
                self.iou.update(preds, gts)
