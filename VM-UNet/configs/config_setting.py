...
class setting_config:
    ...
    network = 'vmunet'
    model_config = {
        'num_classes': 1, #? Why is this 1 and not 2
        'input_channels': 3, 
        # ----- VM-UNet ----- #
        'depths': [2,2,2,2],
    ...
    if datasets == 'isic18':
        ...
        floga_configs = '/mnt/VM-UNet/configs/FLOGA_configs.json'
        sen2_bands_idx = [1, 2, 3, 4, 5, 6, 8, 9, 10]
    else:
        raise Exception('config.datasets is not a valid option!')
    ...
    test_transformer = transforms.Compose([
        myNormalize(datasets, train=False), #? How does this affect the training
        myToTensor(),
        myResize(input_size_h, input_size_w)
    ])
