...
configs['method'] = 'changeformer'
model_configs = pyjson5.load(open(Path('/mnt/FLOGA/FLOGA-github/configs') / 'method' / f'{configs["method"]}.json', 'r'))
configs['mode'] = 'eval'
configs['paths']['load_state'] = '/mnt/FLOGA/data/results/changeformer/20240709104027/checkpoints/0/best_segmentation.pt'

run_path, resume_from_checkpoint, _ = resume_or_start(configs, model_configs)
results_path = Path('/mnt/FLOGA/gradio_outputs/changeformer/')
...
def generate_mask():
    ...
    before_img = np.expand_dims(np.load(before_img_f), axis=0).astype(np.float32, copy=False)
    before_img_f.close()

    after_img = np.expand_dims(np.load(after_img_f), axis=0).astype(np.float32, copy=False)
    after_img_f.close()
    
    before_img = torch.from_numpy(before_img).to(torch.float32, copy=False)
    after_img = torch.from_numpy(after_img).to(torch.float32, copy=False)

    label = None
    if label_f is not None:
        label = np.load(label_f).astype(np.float32, copy=False)
        label_f.close()
    ...
    before_img = before_img[:, bands, :, :].to(device, copy=False)
    after_img = after_img[:, bands, :, :].to(device, copy=False)
    
    with torch.no_grad():
        output = model(before_img, after_img)
    output = output[-1]
    predictions = output.argmax(1).to(dtype=torch.int8, copy=False)
    print(np.unique(predictions.squeeze().numpy(force=True), return_counts=True))
    ...
    predictions = predictions.squeeze().numpy(force=True)
