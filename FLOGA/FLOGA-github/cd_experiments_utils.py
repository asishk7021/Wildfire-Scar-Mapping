...
def train_change_detection(model, device, class_weights, run_path, init_epoch, train_loader, val_loader, validation_id,
    ...
    for epoch in range(init_epoch, last_epoch):
        ...
        with open(run_path / 'checkpoints' / f'{rep_i}' / 'training_logs.txt', 'a') as f:
            f.write(f'\n===== Epoch {epoch} =====\n')
            f.write(f'Average train loss per image: {train_loss / configs["datasets"]["batch_size"]}\n')
            f.write(f'F1-score: {score[1].item()}\n')
        ...
        avg_val_loss, val_acc, val_score, miou, burnt_score = eval_change_detection(model, device, class_weights,
        ...
        with open(run_path / 'checkpoints' / f'{rep_i}' / 'validation_logs.txt', 'a') as f:
            f.write(f'\n===== Epoch {epoch} =====\n')
            f.write(f'Average validation loss per image: {avg_val_loss}\n')
            f.write(f'Average accuracy loss: {val_acc}\n')
            f.write(f'Average validation F1-score: {val_score}\n')
...
def eval_change_detection(model, device, class_weights, init_epoch, loader, validation_id, gsd,
    ...
    if mode == 'test':
        ...
        return total_loss/(len(loader) * configs["datasets"]["batch_size"]), 100 * acc.nanmean(), 100 * score.nanmean(), 100 * mean_iou, score[1]
