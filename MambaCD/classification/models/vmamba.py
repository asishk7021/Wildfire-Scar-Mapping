...
class VSSM(nn.Module):
    def __init__(
        self, 
        patch_size=4, 
        in_chans=9, #HACK change here
        num_classes=2, #HACK change here
        depths=[2, 2, 9, 2], 
        dims=[96, 192, 384, 768], 
        # =========================
        ssm_d_state=16,
        ...
        **kwargs,
        ...
        super().__init__()
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"]) #* Evaluates to False (norm_layer = 'ln')
        ...
        ).get(patchembed_version, None) #* patchembed_version set to v2
    ...
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        ...
        return nn.Sequential(
            ...
            (nn.Identity() if (channel_first or (not patch_norm)) else Permute(0, 2, 3, 1)), # (B, W, C, H)
            (norm_layer(embed_dim // 2) if patch_norm else nn.Identity()),
            (nn.Identity() if (channel_first or (not patch_norm)) else Permute(0, 3, 1, 2)), # (B, H, W, C)
            nn.GELU(),
            nn.Conv2d(embed_dim // 2, embed_dim, kernel_size=3, stride=2, padding=1),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)), # (B, W, C, H)
            (norm_layer(embed_dim) if patch_norm else nn.Identity()),
        )
