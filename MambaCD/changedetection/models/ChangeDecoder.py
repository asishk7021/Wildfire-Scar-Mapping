...
from MambaCD.changedetection.models.triplet_attention import TripletAttention
...
class ChangeDecoder(nn.Module):
    ...
    def _upsample_add(self, x, y):
        _, _, H, W = y.size()
        return self._apply_triplet_attention(F.interpolate(x, size=(H, W), mode='bilinear')) + y
    
    def _apply_triplet_attention(self, tensor):
        device = tensor.device
        
        att = tensor.detach().clone()
        att = att.cuda(device)

        tripletAttentionLayer = TripletAttention()
        tripletAttentionLayer.cuda(device)
        att = tripletAttentionLayer(att)
        
        return tensor + att
    
    def forward(self, pre_features, post_features):
        pre_feat_1, pre_feat_2, pre_feat_3, pre_feat_4 = (self._apply_triplet_attention(x) for x in pre_features)
        post_feat_1, post_feat_2, post_feat_3, post_feat_4 = (self._apply_triplet_attention(x) for x in post_features)
        ...
        p4 = self._apply_triplet_attention(p4)
        ...
        # p3 = self._apply_triplet_attention(p3)
        ...
        # p2 = self._apply_triplet_attention(p2)
        ...
        # p1 = self._apply_triplet_attention(p1)
