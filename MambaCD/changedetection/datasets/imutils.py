...
def random_fliplr(pre_img, post_img, label): #TODO check how this affects 3D array
...
def random_flipud(pre_img, post_img, label): #TODO check how this affects 3D array
    if random.random() > 0.5:
...
def random_rot(pre_img, post_img, label): #TODO check how this affects 3D array
...
def random_crop_new(pre_img, post_img, label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    ...
    pad_pre_image = np.zeros((H, W, 3), dtype=np.float32) #TODO set column 2 to 9 instead of 3

    pad_post_image = np.zeros((H, W, 3), dtype=np.float32) #TODO set column 2 to 9 instead of 3
    pad_label = np.ones((H, W), dtype=np.float32) * ignore_index

    # pad_pre_image[:, :] = mean_rgb[0]
    #* Below ops are not necessary for 9ch inputs.
